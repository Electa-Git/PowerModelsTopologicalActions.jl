...
function run_acdcsw_AC_grid_indicator(file, model_constructor, optimizer; kwargs...)
    return _PM.solve_model(file, model_constructor, optimizer, build_acdcsw_AC_grid_indicator; ref_extensions=[_PM.ref_add_on_off_va_bounds!], kwargs...)
end

""
function build_acdcsw_AC_grid_indicator(pm::_PM.AbstractPowerModel)
    # AC grid
    _PM.variable_bus_voltage(pm)
    _PM.variable_gen_power(pm)
    _PM.variable_branch_power(pm)
    ...
    for i in _PM.ids(pm, :switch)
        constraint_switch_thermal_limit(pm, i) # limiting the apparent power flowing through an ac switch
        constraint_switch_voltage_on_off_indicator(pm,i) # making sure that the voltage magnitude and angles are equal at the two extremes of a closed switch
        constraint_switch_power_on_off(pm,i) # limiting the maximum active and reactive power through an ac switch
    end
