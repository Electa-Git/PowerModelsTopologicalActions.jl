...
function run_acdcsw_AC_droop(file, model_constructor, optimizer; kwargs...)
    ...
    return _PM.solve_model(file, model_constructor, optimizer, build_acdcsw_AC_droop; ref_extensions=[_PMACDC.add_ref_dcgrid!, _PMACDC.ref_add_pst!, _PMACDC.ref_add_sssc!, _PMACDC.ref_add_gendc!,_PM.ref_add_on_off_va_bounds!], kwargs...)
end

""
function build_acdcsw_AC_droop(pm::_PM.AbstractPowerModel)
    # AC grid
    ...
    for i in _PM.ids(pm, :convdc)
        ...
        if _PM.ref(pm, :convdc, i)["type_ac"] == 2
            _PMACDC.constraint_ac_voltage_droop_control(pm, i)
        end
    end
end