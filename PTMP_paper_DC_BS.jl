...
data_busbars_ac_split_5_acdc,  switches_couples_ac_5,  extremes_ZILs_5_ac  = _PMTP.DC_busbar_split_more_buses(data_busbars_ac_split_5_acdc,splitted_bus_ac)
data_busbars_ac_split_5_acdc_plus,  switches_couples_ac_5_plus,  extremes_ZILs_5_ac_plus  = _PMTP.DC_busbar_split_more_buses(data_busbars_ac_split_5_acdc_plus,splitted_bus_ac)

# Duplicating the network data
for (sw_id,sw) in data_busbars_ac_split_5_acdc["dcswitch"]
    if sw_id == "1"
        sw["cost"] = 1.0
    else
        sw["cost"] = 0.0
...
result_switches_AC_ac_ref  = _PMTP.run_acdcsw_DC_big_M_ZIL(ac_bs_ac_ref,ACPPowerModel,juniper)
result_switches_lpac_ac_ref  = _PMTP.run_acdcsw_DC_big_M_ZIL(ac_bs_lpac_ref,LPACCPowerModel,gurobi_opf)
